...
case class CollectHandles(child: Expression,
  ...
  override def initialize(mutableAggBuffer: MutableRow): Unit = buffer.clear()
  def initialize(b: InternalRow): Unit =
    buffer.clear()

  override def update(mutableAggBuffer: MutableRow, input: InternalRow): Unit = {
    val value = child.eval(input)
    if (value != null) {
      buffer.addAll(value.asInstanceOf[GenericArrayData].toLongArray())
    }
  }
  def update(b: InternalRow, input: InternalRow): Unit = {
    val value = child.eval(input)
    if (value != null) {
      buffer.addAll(value.asInstanceOf[GenericArrayData].toLongArray())
    }
  ...
  def merge(buffer: InternalRow, input: InternalRow): Unit =
