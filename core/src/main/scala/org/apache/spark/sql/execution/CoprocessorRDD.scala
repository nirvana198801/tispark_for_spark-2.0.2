...
case class CoprocessorRDD(output: Seq[Attribute], tiRdd: TiRDD) extends LeafExecNode {
  ...
  protected override def doExecute(): RDD[InternalRow] = {
    ...
    internalRdd.mapPartitionsInternal { iter =>
...
case class RegionTaskExec(child: SparkPlan,
  ...
  def rowToInternalRow(row: Row,
    ...
    val mutableRow = new GenericInternalRow(
      outputTypes.indices.map(x => converters(x)(row(x))).toArray
    )
    mutableRow
  }
        ...
        iter.flatMap { row =>
          ...
          val resultIter = new util.Iterator[UnsafeRow] {
            ...
            override def next(): UnsafeRow = {
              ...
              //project.initialize(index)
              val prj = UnsafeProjection.create(schema)
              ...
              prj(rowToInternalRow(sparkRow, outputTypes, converters))
            }
          }
          resultIter
        }
